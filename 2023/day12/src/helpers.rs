...
pub fn parse_into_two_fields<T1, T2, const SEP: char>(input: &str) -> (T1, T2)
where
    T1: Copy + std::str::FromStr,
    <T1 as std::str::FromStr>::Err: std::fmt::Debug,
    T2: Copy + std::str::FromStr,
    <T2 as std::str::FromStr>::Err: std::fmt::Debug,
{
    let split = input.split(SEP).collect::<Vec<&str>>();
    assert_eq!(split.len(), 2, "need two fields");
    let f1 = split[0].parse::<T1>().unwrap();
    let f2 = split[0].parse::<T2>().unwrap();
    (f1, f2)
}
