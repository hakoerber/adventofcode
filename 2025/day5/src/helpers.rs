...
impl Point {
    ...
    pub fn adjacents(&self) -> impl Iterator<Item = Self> {
        self.all_adjacents().flatten()
    }

    pub fn all_adjacents(&self) -> impl Iterator<Item = Option<Self>> {
        let mut v = Vec::new();
        for x in -1..=1 {
            for y in -1..=1 {
                if (x, y) != (0, 0) {
...
impl<T> Grid<T> {
    ...
    pub fn adjacents_of<'a>(&'a self, point: &Point) -> impl Iterator<Item = GridPoint<'a, T>> {
        let mut v = Vec::new();
    ...
    pub fn iter(&self) -> impl Iterator<Item = GridPoint<'_, T>> {
        GridIterRef {
            point: Point { x: 0, y: 0 },
            grid: self,
        }
    ...
    pub fn filter_points<F>(&self, f: F) -> impl Iterator<Item = Point>
    where
        F: Fn(&T) -> bool,
    {
        self.iter()
            .filter_map(move |elem| f(elem.value).then_some(elem.point))
    }
}
...
where
    T: Copy,
{
    pub fn iter_copy(&self) -> impl Iterator<Item = GridPointOwned<T>> {
        GridIterCopy {
            point: Point { x: 0, y: 0 },
            grid: self,
        }
    ...
    pub fn filter_points_copy<F>(&self, f: F) -> impl Iterator<Item = Point>
    where
        F: Fn(T) -> bool,
    {
        self.iter_copy()
    ...
    pub fn adjacents_of_copy(&self, point: &Point) -> impl Iterator<Item = GridPointOwned<T>> {
        let mut v = Vec::new();
        for point in point.adjacents() {
            if let Some(value) = self.get(&point) {
                v.push(GridPointOwned {
...
pub struct GridIterCopy<'a, T> {
    point: Point,
    grid: &'a Grid<T>,
}

impl<T> Iterator for GridIterCopy<'_, T>
where
    T: Copy,
{
    type Item = GridPointOwned<T>;

    fn next(&mut self) -> Option<Self::Item> {
        assert!(self.point.y <= self.grid.height());
        let elem = self.grid.get(&self.point).map(|e| GridPointOwned {
...
pub struct GridIterRef<'a, T> {
    point: Point,
    grid: &'a Grid<T>,
}

impl<'a, T> Iterator for GridIterRef<'a, T> {
    type Item = GridPoint<'a, T>;

    fn next(&mut self) -> Option<Self::Item> {
        assert!(self.point.y <= self.grid.height());
        let elem = self.grid.get(&self.point).map(|e| GridPoint {
            point: self.point.clone(),
            value: e,
        });
