...
pub use output::{Output, OutputConst};
...
mod test {
    ...
    fn example_part_1() {
        if let Some(outputs) = &puzzle::EXAMPLE_RESULTS[0] {
            for (num, expected) in outputs.iter().enumerate() {
                ...
                let expected: Output = expected.clone().into();
    ...
    fn example_part_2() {
        if let Some(outputs) = &puzzle::EXAMPLE_RESULTS[1] {
            for (_num, expected) in outputs.iter().enumerate() {
                let output = puzzle::part_2(&example_input(2));
                let expected: Output = expected.clone().into();
                assert_eq!(output, expected);
            }
        }
